<template>
  <div class="article-wrapper">
    ...
    <transition name="fade">
      <ul class="article-list">
        <li v-for="article of articleList" :key="article.id" class="article-item">
          <div class="article-content" @click="navigateToDetail(article.id)">
    ...
    <div class="loading-more-tip" v-loading="loadingMore" v-if="loadingMore"></div>
    <div class="nomore-tip" v-if="nomoreTipShowable">---到底咯---</div>
    <div class="nodata-tip" v-if="articleList.length === 0 && !loading">---暂无数据---</div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    navigateToDetail(id) {
      this.$router.push(`/${this.blogOrEssay}/${id}`);
      this.changeBlogOrEssay({ blogOrEssay: this.blogOrEssay });
    }
  },
  created() {
    this.getHomeArticleList(this.pageIndex);
